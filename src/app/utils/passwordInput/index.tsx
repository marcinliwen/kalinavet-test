...
type PasswordInput = {
   id: string, title: string, defaultValue: string, name: string, onChange: () => void, error?: any, autoFocus: boolean
}

export default function PasswordInput({ id, name, autoFocus, onChange, error, title }: PasswordInput) {
    const t = useTranslations('Settings');
    const [togglePassword, setTogglePassword] = React.useState<boolean>(false)

    return (
        <div className="mb-6 relative">
       <label className='min-w-32 md:text-right text-sm'>{title}</label>
       <div className='relative'>
        ...
        {error && <span className="text-red-600 block absolute text-xs -bottom-6 px-2">{error.message}</span>}
