...
//console.log('state', errors)
   ...
   const [toggleConfirmation, setToggleConfirmation] = React.useState<boolean>(false)
    ...
    const [submitDisabled, setSubmit] = React.useState<boolean>(true)
    ...
    function onInputChange(e:any){
        ...
        if(!value){
            setError({...error, confirmation: 'Musisz potwierdzić hasło'})
            
        }
        else if(password && value !== password){
            setError({...error, confirmation:'Hasło musi być identyczne'})
        }
        else{
    ...
    function onPasswordInput(e:any){
        ...
        if(value.length > 7 ){
            ...
            setError({...error, password: 'Hasło musi mieć co najmniej 8 znaków'})
        }else{
            setError({...error, password: 'Podaj nowe hasło'})
        }
    }
    return(
